...
async function processActivityPush(activity: GarminActivityPush): Promise<void> {
  ...
  const distanceMiles = distanceInMeters ? distanceInMeters * 0.000621371 : 0;
  const elevationGainFeet = totalElevationGainInMeters ? totalElevationGainInMeters * 3.28084 : 0;
  const startTime = new Date(startTimeInSeconds * 1000);
