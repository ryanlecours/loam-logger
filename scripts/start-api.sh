...
# fails, the server won't start — better than booting with a schema
# mismatch and throwing at runtime query time.
#
# Using the local binary (not `npx prisma`) avoids npx's version-resolution
