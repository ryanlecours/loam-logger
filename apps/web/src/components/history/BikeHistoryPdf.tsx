...
import { GARMIN_INSIGHT_ATTRIBUTION, GARMIN_TRADEMARK_NOTICE } from '@loam/shared';
...
const styles = StyleSheet.create({
  ...
  // Sits below the generated-by line, repeated on every page.
  attributionFooter: { position: 'absolute', left: 36, right: 36, bottom: 4, fontSize: 6, color: MUTED },
  trademark: { marginTop: 1 },
...
export function BikeHistoryPdf({ bike, totals, yearGroups, distanceUnit, timeframeLabel, truncated, logoSrc }: Props) {
  ...
  // Only attribute when Garmin actually contributed — the guidelines forbid
  // the mark and the attribution where its data is absent.
  const hasGarmin = bike.contributingSources?.includes('garmin') ?? false;
  return (
    <Document title={`${bikeName(bike)} – Loam Logger history`}>
      <Page size="LETTER" style={styles.page} wrap>
        ...
        {/* `fixed` renders this on every page. That is a requirement, not a
            nicety: an exported PDF is downstream data under the Garmin API
            Brand Guidelines, which state the attribution "must be adjacent to
            the data and repeated on each page". */}
        <View style={styles.footer} fixed>
          <Text>Generated by Loam Logger · {generated.toLocaleDateString()}</Text>
          <Text render={({ pageNumber, totalPages }) => `${pageNumber} / ${totalPages}`} />
        </View>
        {hasGarmin && (
          <View style={styles.attributionFooter} fixed>
            <Text>{GARMIN_INSIGHT_ATTRIBUTION}</Text>
            <Text style={styles.trademark}>{GARMIN_TRADEMARK_NOTICE}</Text>
          </View>
        )}
      </Page>
    </Document>
